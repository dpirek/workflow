...
export class WorkflowRepository {
  ...
  nextDefinitionVersion(key) {
    return Number(this.db.prepare(
      'SELECT COALESCE(MAX(version), 0) + 1 AS version FROM process_definition WHERE process_key = ?'
    ).get(key).version);
  }

  insertNode(definitionId, node) {
    const config = { ...(node.config || {}) };
    for (const field of ['workerType', 'assignee', 'candidateGroup', 'priority', 'dueAt', 'durationMs', 'retries', 'messageName', 'correlationKey']) {
  ...
  insertEdge(definitionId, edge, nodeIds) {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(definitionId, edge.key ?? null, nodeIds.get(edge.from), nodeIds.get(edge.to), edge.condition ?? null, edge.priority ?? 0);
  }
  ...
  findDefinition(key, version) {
    if (version !== undefined && version !== null) {
      return this.db.prepare('SELECT * FROM process_definition WHERE process_key = ? AND version = ?').get(key, version);
    }
  ...
  definitionGraph(definitionId) {
    return {
      nodes: this.db.prepare('SELECT * FROM process_node WHERE process_definition_id = ? ORDER BY id').all(definitionId),
  ...
  startNode(definitionId) {
    return this.db.prepare("SELECT * FROM process_node WHERE process_definition_id = ? AND node_type = 'START'").get(definitionId);
  }

  node(id) { return this.db.prepare('SELECT * FROM process_node WHERE id = ?').get(id); }
  outgoingEdges(nodeId) {
    return this.db.prepare('SELECT * FROM process_edge WHERE source_node_id = ? ORDER BY priority DESC, id ASC').all(nodeId);
  }
  incomingCount(nodeId) {
    return Number(this.db.prepare('SELECT COUNT(*) AS count FROM process_edge WHERE target_node_id = ?').get(nodeId).count);
  }
  ...
  instance(id) { return this.db.prepare('SELECT * FROM process_instance WHERE id = ?').get(id); }
  updateInstanceStatus(id, status, ended = false) {
    this.db.prepare(`UPDATE process_instance SET status = ?, ended_at = ${ended ? 'CURRENT_TIMESTAMP' : 'NULL'} WHERE id = ?`).run(status, id);
  }
  listInstances({ status, processKey, businessKey, limit = 100 }) {
    const clauses = [];
    const params = [];
    if (status) { clauses.push('i.status = ?'); params.push(status); }
    if (processKey) { clauses.push('d.process_key = ?'); params.push(processKey); }
    if (businessKey) { clauses.push('i.business_key = ?'); params.push(businessKey); }
  ...
  token(id) { return this.db.prepare('SELECT * FROM token WHERE id = ?').get(id); }
  moveToken(id, nodeId) { this.db.prepare("UPDATE token SET node_id = ?, state = 'ACTIVE' WHERE id = ?").run(nodeId, id); }
  setTokenState(id, state) {
    this.db.prepare(`UPDATE token SET state = ?, completed_at = ${state === 'COMPLETED' ? 'CURRENT_TIMESTAMP' : 'NULL'} WHERE id = ?`).run(state, id);
  }
  activeTokenCount(instanceId) {
    return Number(this.db.prepare("SELECT COUNT(*) AS count FROM token WHERE process_instance_id = ? AND state IN ('ACTIVE','WAITING')").get(instanceId).count);
  }
  waitingTokensAt(instanceId, nodeId) {
    return this.db.prepare("SELECT * FROM token WHERE process_instance_id = ? AND node_id = ? AND state = 'WAITING' ORDER BY id").all(instanceId, nodeId);
  }
  ...
  cancelTokens(instanceId) {
    this.db.prepare("UPDATE token SET state = 'CANCELLED', completed_at = CURRENT_TIMESTAMP WHERE process_instance_id = ? AND state IN ('ACTIVE','WAITING')").run(instanceId);
  }
  ...
  activeTaskFor(tokenId, nodeId) {
    return this.db.prepare("SELECT * FROM task WHERE token_id = ? AND node_id = ? AND status IN ('CREATED','CLAIMED') ORDER BY id DESC LIMIT 1").get(tokenId, nodeId);
  }
  insertTask(token, node, config) {
    const result = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(token.process_instance_id, token.id, node.id, node.node_key, node.name || node.node_key,
      config.assignee ?? null, config.candidateGroup ?? null, config.priority ?? 50, config.dueAt ?? null);
    return Number(result.lastInsertRowid);
  }
  task(id) { return this.db.prepare('SELECT * FROM task WHERE id = ?').get(id); }
  ...
  listTasks({ status, assignee, candidateGroup, instanceId, limit = 100 }) {
    ...
    if (candidateGroup) { clauses.push('t.candidate_group = ?'); params.push(candidateGroup); }
    if (instanceId) { clauses.push('t.process_instance_id = ?'); params.push(instanceId); }
    params.push(Math.min(Math.max(limit, 1), 500));
    return this.db.prepare(`
  ...
  cancelTasks(instanceId) { this.db.prepare("UPDATE task SET status = 'CANCELLED', completed_at = CURRENT_TIMESTAMP WHERE process_instance_id = ? AND status IN ('CREATED','CLAIMED')").run(instanceId); }

  activeJobFor(tokenId, nodeId) {
    return this.db.prepare("SELECT * FROM job WHERE token_id = ? AND node_id = ? AND status IN ('READY','RUNNING') ORDER BY id DESC LIMIT 1").get(tokenId, nodeId);
  }
  ...
  job(id) { return this.db.prepare('SELECT * FROM job WHERE id = ?').get(id); }
  readyServiceJobs(workerTypes, limit, now) {
    const filter = workerTypes?.length ? `AND json_extract(j.payload_json, '$.workerType') IN (${placeholders(workerTypes)})` : '';
    return this.db.prepare(`
  ...
  completeJob(id) {
    this.db.prepare("UPDATE job SET status = 'COMPLETED', completed_at = CURRENT_TIMESTAMP, locked_by = NULL, locked_until = NULL WHERE id = ?").run(id);
  }
  retryJob(id, retries, dueAt, error = null) {
    this.db.prepare("UPDATE job SET status = 'READY', retries = ?, due_at = ?, last_error = ?, locked_by = NULL, locked_until = NULL WHERE id = ?").run(retries, dueAt, error, id);
  }
  deadJob(id, error) {
    this.db.prepare("UPDATE job SET status = 'DEAD', retries = 0, last_error = ?, locked_by = NULL, locked_until = NULL WHERE id = ?").run(error, id);
  }
  ...
  jobsForInstance(instanceId) { return this.db.prepare('SELECT * FROM job WHERE process_instance_id = ? ORDER BY id').all(instanceId); }
  cancelJobs(instanceId) { this.db.prepare("UPDATE job SET status = 'DEAD', last_error = 'Process cancelled', locked_by = NULL, locked_until = NULL WHERE process_instance_id = ? AND status IN ('READY','RUNNING','FAILED')").run(instanceId); }
  ...
  incident(id) { return this.db.prepare('SELECT * FROM incident WHERE id = ?').get(id); }
  resolveIncident(id) { return this.db.prepare("UPDATE incident SET status = 'RESOLVED', resolved_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'OPEN'").run(id).changes; }
  listIncidents(status, instanceId, limit = 100) {
    const clauses = [], params = [];
    if (status) { clauses.push('status = ?'); params.push(status); }
    if (instanceId) { clauses.push('process_instance_id = ?'); params.push(instanceId); }
    params.push(Math.min(Math.max(limit, 1), 500));
    return this.db.prepare(`SELECT * FROM incident ${clauses.length ? `WHERE ${clauses.join(' AND ')}` : ''} ORDER BY id DESC LIMIT ?`).all(...params);
  }
  ...
  history(instanceId, type, limit = 500) {
    ...
    return this.db.prepare(`SELECT * FROM history_event WHERE process_instance_id = ? ${filter} ORDER BY id ASC LIMIT ?`).all(...params);
  }
  addHistory(instanceId, type, refs = {}, data = {}) {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(instanceId, refs.nodeId ?? null, refs.tokenId ?? null, refs.taskId ?? null, refs.jobId ?? null, type, JSON.stringify(data));
  }
}
