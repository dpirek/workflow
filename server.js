...
const sessionToken = (user) => { const value = `${user.id}.${randomBytes(18).toString('hex')}`; return `${value}.${createHmac('sha256', secret).update(value).digest('hex')}`; };
function currentUser(req) { const raw = (req.headers.cookie || '').split(';').map((x) => x.trim()).find((x) => x.startsWith('workflow_session='))?.split('=')[1]; return raw && sessions.get(raw) || null; }
function saveUsers() { writeFileSync(usersPath, JSON.stringify(users, null, 2)); }
function requireUser(req, res) { const user = currentUser(req); if (!user) json(res, 401, { error: 'Authentication required' }); return user; }
function safePath(urlPath) { const file = normalize(join(publicRoot, urlPath === '/' ? 'index.html' : urlPath)); return file.startsWith(publicRoot) ? file : null; }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === '/api/auth/register' && req.method === 'POST') {
      const { name, email, password } = await body(req); const normalized = String(email || '').trim().toLowerCase();
      if (!name || !normalized || !password || password.length < 8) return json(res, 400, { error: 'Name, email, and a password of at least 8 characters are required' });
      if (users.some((item) => item.email === normalized)) return json(res, 409, { error: 'An account with that email already exists' });
      const salt = randomBytes(16).toString('hex'); const userData = { id: randomBytes(8).toString('hex'), name: String(name).trim(), email: normalized, role: 'user', salt, hash: scryptSync(password, salt, 64).toString('hex'), createdAt: new Date().toISOString() }; users.push(userData); saveUsers();
      const session = sessionToken(userData); const safe = { id: userData.id, name: userData.name, email: userData.email, role: userData.role }; sessions.set(session, safe); return json(res, 201, { user: safe }, { 'set-cookie': `workflow_session=${session}; HttpOnly; Path=/; SameSite=Lax; Max-Age=604800` });
    }
    if (path === '/api/auth/login' && req.method === 'POST') {
      const { email, password } = await body(req); const found = users.find((item) => item.email === String(email || '').trim().toLowerCase());
      const hash = found && scryptSync(password || '', found.salt, 64); if (!found || !timingSafeEqual(Buffer.from(found.hash, 'hex'), hash)) return json(res, 401, { error: 'Invalid email or password' });
      const session = sessionToken(found); const safe = { id: found.id, name: found.name, email: found.email, role: found.role }; sessions.set(session, safe); return json(res, 200, { user: safe }, { 'set-cookie': `workflow_session=${session}; HttpOnly; Path=/; SameSite=Lax; Max-Age=604800` });
    }
...
const port = Number(process.env.PORT || 8080); server.listen(port, () => console.log(`Workflow web app listening on http://127.0.0.1:${port}`));
process.on('SIGTERM', () => { engine.close(); server.close(); }); process.on('SIGINT', () => { engine.close(); server.close(); });
