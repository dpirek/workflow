...
test('serves workflow tools through Streamable HTTP MCP', async () => {
  ...
  try {
    ...
    const deploy = await rpc(http.url, 'tools/call', {
      name: 'deploy_workflow', arguments: {
        definition: {
    ...
    const start = await rpc(http.url, 'tools/call', { name: 'start_process', arguments: { processKey: 'http-flow', variables: {} } });
...
async function rpc(base, method, params) {
  const response = await fetch(`${base}/mcp`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ jsonrpc: '2.0', id: Math.random(), method, params }) });
